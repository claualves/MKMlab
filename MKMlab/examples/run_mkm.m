...
[soln_struct, ODE_sol] = ODE_solver(eqn_handle_str,y0,options);
...
y_gas_species = [];
if CPT == true
    [rxn_struct.y_site_species] = ODE_sol.y;
    [soln_struct.ODE_sol] = ODE_sol;
    for i = 1:size(ODE_sol.y,2)
    theta_tmp = ODE_sol.y(:,i);
    y_gas_species_temp =  calculate_TOF(TOF_eqn_handle_str,theta_tmp);
    y_gas_species = [y_gas_species y_gas_species_temp];
    end   
    [rxn_struct.y_gas_species] = y_gas_species;
end
